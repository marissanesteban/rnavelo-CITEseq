...
ggsave(
  ...
  plot = p,
  width = 18,        # inches — makes LOTS of pixels
  height = 8,
...
# THIS INCLUDES BASAL I IN THE HF KERATINOCYTES
...
ker_uw$Keratinocyte_Lineage <- ifelse(
  ker_uw$celltype %in% HF_keratinocytes_2,
  "HF_Keratinocyte",
  "IFE_Keratinocyte"
)
...
# WOUDNED
ker_wo$Keratinocyte_Lineage <- ifelse(
  ker_wo$celltype %in% HF_keratinocytes_2,
  "HF_Keratinocyte",
  "IFE_Keratinocyte"
)
...
```{r}
DimPlot(IFE_WO, reduction = "umap.harmony", group.by = "celltype")
DimPlot(IFE_WO, reduction = "umap.harmony", group.by = "orig.ident", split.by = "orig.ident")
```


```{r}
saveRDS(IFE_UW, file = "/Volumes/PortableSSD/SRSP/RNAVelo_sc/keratinocytes/UW_IFE.rds")

saveRDS(IFE_WO, file = "/Volumes/PortableSSD/SRSP/RNAVelo_sc/keratinocytes/WO_IFE.rds")
```
